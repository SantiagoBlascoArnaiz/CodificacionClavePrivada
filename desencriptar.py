...
def inversaCuerpo(matriz):
    ...
    for i in range(len(matriz)):
        for j in range(len(matriz[i])):
            ...
            matrizCopia[i][j] = multCuerpo(det, determinanteCuerpo(menor))
    matrizCopia = trasponerMatriz(matrizCopia)
    return matrizCopia
    
##############################################################################

def invPermutaFilas(cadena):
...
def invSubElements(cadena,ronda):
    cadenaSub = ''
    if(ronda==0):
        for i in cadena:
...
def invMixColumns(cadena, ronda):
    if(ronda==0):
    ...
    inv = inversaCuerpo(matriz)
    vectores = multMatCuerpo(inv, cadenaMatCuerpo(cadena))

    return matCadenaCuerpo(vectores)
    
def invSumaClave(cadena):
    cadenaResult = ''
    for i in range(len(cadena)):
        cadenaResult = cadenaResult + (binHex(sumaCuerpo(hexBin(clave[i]), hexBin(cadena[i]))))
    return cadenaResult

def desEncripta(cadena):
    cadEnc = invMixColumns(cadena, 1)
    #print(cadEnc,'mix')
    cadEnc = invPermutaFilas(cadEnc)
    #print(cadEnc,'permuta')
    cadEnc = invSubElements(cadEnc, 1)
    #print(cadEnc,'sub')
    
    #print()
    
    cadEnc = invSumaClave(cadEnc)
    #print(cadEnc,'suma')
    cadEnc = invMixColumns(cadEnc, 0)
    #print(cadEnc,'mix')
    cadEnc = invPermutaFilas(cadEnc)
    #print(cadEnc,'permuta')
    cadEnc = invSubElements(cadEnc, 0)
    #print(cadEnc,'sub')
    #print()
...
if __name__ == "__main__":
    ...
    biy0 = dict(zip(biyeccion0(),cadenaHex))
    biy1 = dict(zip(biyeccion1(),cadenaHex))
